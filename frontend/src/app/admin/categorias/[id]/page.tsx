...
export default function EditarCategoriaPage() {
  ...
  useEffect(() => {
    if (id) {
      Promise.all([cargarCategorias(), cargarCategoria()]);
    }
  }, [id]);
  ...
  const cargarCategoria = async () => {
    ...
    try {
      const categoria: any = await categoriasAPI.obtener(id);
      setForm({
      ...
      if (categoria.imagen) {
        ...
        setTipoImagen(categoria.imagen.startsWith('data:') ? 'archivo' : 'url');
      }
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setCargando(true);
    try {
      await categoriasAPI.actualizar(id, {
        ...form,
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold font-montserrat text-azul-oscuro flex items-center gap-2">
                <Folder size={22} className="text-teal" /> Editar Categoría
              </h1>
            </div>
          </div>
        </div>
            ...
            <div className="flex justify-end gap-4 pt-4 border-t border-gray-100">
              ...
              >
                {cargando ? (
                  ...
                  <><Save size={18} /> Actualizar Categoría</>
                )}
              </button>
            </div>
          </form>
