...
function NuevaTallaContent() {
  ...
  const [form, setForm] = useState({
    nombre: '',
    orden: 0,
    coleccionId: coleccionId ? Number(coleccionId) : null,
  });
  ...
  const cargarColecciones = async () => {
    try {
      setCargando(true);
      const data = await sizeCollectionsAPI.listar();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setGuardando(true);
      await sizesAPI.crear(form);
      toast.success('Talla creada correctamente');
      router.push('/admin/tallas');
    } catch (error: any) {
      console.error(error);
      toast.error(error?.response?.data || 'Error al crear la talla');
    } finally {
      setGuardando(false);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold font-montserrat text-azul-oscuro flex items-center gap-2">
                <Ruler size={22} className="text-teal" /> Nueva Talla
              </h1>
            </div>
          </div>
        </div>
